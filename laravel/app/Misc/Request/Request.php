...
class Request extends BaseRequest
{
    ...
    public function request($method, $url, $query = [], $post = [], $attach = [])
    {
        ...
        try {
            $this->response = $this->send($method, $url, $this->options, $query, $post, $attach, $this->cookies);
            $this->url = $this->response->effectiveUri();
    ...
    protected function addResponseInfoToError()
    {
        ...
        $this->error .= ': ' .$this->response->getReasonPhrase() . ' ' . $this->response->effectiveUri();
    ...
    public function validateJson($required, $optional = [], $canHaveOtherFields = true)
    {
        ...
        if ($error) {
            if (!empty($data['missingRequired'])) {
                $errors[] = trans_choice(
                    'В JSON-ответе отсутствует :fields|В JSON-ответе отсутствуют :fields|В JSON-ответе отсутствуют :fields',
                    $data['missingRequired'],
                    ['fields' => implode(', ', array_keys($data['missingRequired']))],
                    'ru'
                );
            }
